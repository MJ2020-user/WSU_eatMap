...
for (const button of buttons) {
  button.addEventListener("click", function(event) {
    ...
    const restaurantTime = "영업시간: 10:00 AM - 9:00 PM"; // 식당 운영 시간 정보를 문자열로 설정
    ...
    geocoding(restaurantName, restaurantAddress, restaurantTime);  // geocoding 함수를 호출하여 주소를 좌표로 변환하고 지도를 생성합니다.
  })
}
...
function mapGenerator(name, la, lo ,restaurantTime){
  ...
  if (isMobile()) {
    ...
    var contentString = [
        '<div>',
        '   <h5>'+name+'</h5>',
        '   <p>영업 시간: ' + restaurantTime + '</p>', // 식당 시간 정보 추가
        '   <a target="_blank" href="http://map.naver.com/search/대전광역시 동구 자양동 '+name+'" >네이버 지도 바로 가기</a>',
        '</div>'
    ].join('');
  }
